...
void Search::addDirichletNoise(const SearchParams& searchParams, Rand& rand, int policySize, float* policyProbs) {
  ...
  for(int i = 0; i<policySize; i++) {
    if(policyProbs[i] >= 0) {
      r[i] = rand.nextGamma(r[i] * searchParams.rootDirichletNoiseTotalConcentration * 0.5);// 降低噪声权重，例如降低到原来的一半
      rSum += r[i];
    }
    else
      r[i] = 0.0;
  ...
  for(int i = 0; i<policySize; i++) {
    if(policyProbs[i] >= 0) {
      double weight = 0.5 * searchParams.rootDirichletNoiseWeight;// 相应调整添加噪声后的策略概率计算
