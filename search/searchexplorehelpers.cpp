...
static double cpuctExploration(double totalChildWeight, const SearchParams& searchParams) {
  return searchParams.cpuctExploration +
    1.7 * searchParams.cpuctExplorationLog * log((totalChildWeight + searchParams.cpuctExplorationBase) / searchParams.cpuctExplorationBase);
}
...
double Search::getExploreScaling(
  ...
  return
    1.5 * cpuctExploration(totalChildWeight, searchParams)
...
double Search::getExploreSelectionValue(
  ...
  double exploreComponent = 1.5 * exploreScaling * nnPolicyProb / (1.0 + childWeight);
  ...
  double valueComponent = pla == P_WHITE ? 0.5 * childUtility : -0.5 * childUtility;
...
double Search::getFpuValueForChildrenAssumeVisited(
  ...
  double fpuValue;
  {
    double fpuReductionMax = isRoot ? searchParams.rootFpuReductionMax - 0.2 : searchParams.fpuReductionMax - 0.1;//ÐÞ¸Ä´¦
