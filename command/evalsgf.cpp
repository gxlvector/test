...
int MainCmds::evalsgf(const vector<string>& args) {
  ...
  for(int moveNum = moveNumStart; moveNum <= moveNumEnd; moveNum++) {
    ...
    if(printScoreNow) {
      ...
      sout << "当前贴目: " << copyHist.rules.komi << endl;
      sout << "白方的奖励分数: " << copyHist.whiteBonusScore << endl;
      sout << "最终的游戏结果: "; WriteSgf::printGameResult(sout, copyHist); sout << endl;
    }
    ...
    sout << "落子所需时间: " << timer.getSeconds() << "s\n";
    sout << "总计算量: " << search->getRootVisits() << "\n";
    sout << "处理的行数: " << nnEval->numRowsProcessed() << endl;
    sout << "处理的批次数量: " << nnEval->numBatchesProcessed() << endl;
    sout << "平均批次大小: " << nnEval->averageProcessedBatchSize() << endl;
